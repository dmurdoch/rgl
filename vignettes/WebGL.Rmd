...
You can swap the order of button and scene; use the `magrittr` dot 
...
NB:  at the time of writing, some unreleased bug fixes
are recommended.  To install the version of `manipulateWidget` used
in this vignette, use
```{r eval = FALSE}
...
Finally, `r indexfns("getElementId")` can be used to extract the 
