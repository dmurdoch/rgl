...
var toggle = function(species) {
  var id = species + `r sphereid - 1`;
  var subscene = `r subid`;
...
<script type="text/javascript">
var toggle = function(species) {
  var id = species + `r rinline("sphereid - 1", script=TRUE)`;
  var subscene = `r rinline("subid", script=TRUE)`;
  if (togglergl.inSubscene(id, subscene))
    togglergl.delFromSubscene(id, subscene)
  else
    togglergl.addToSubscene(id, subscene)
...
- The id of the first set of spheres was stored in `sphereid`, 
so the next ones will be `sphereid + 1` and `sphereid + 2`.
