...
"point\_antialias", "line\_antialias", "depth\_mask", "depth\_test", "polygon_offset", "margin", "floating" and "tag"; 
...
format (or more than one; see below) containing an image to be
...
As mentioned above, `texture` can list more than one file.  This
can be used to specify "mipmaps":  different versions of the
image to use at different magnifications.  Normally OpenGL
rescales the image, averaging nearby pixels to determine
what to display.  However, if `texture` contains several
filenames, these will be used in sequence as the image is 
reduced in size.  Due to limitations in some versions of OpenGL
(including WebGL, used by `r linkfn("rglwidget")`),
the first image *must* have both width and height being an exact
power of two, and `rgl` will stretch it to achieve this.  The
next image cuts both width and height
in half if possible.  For example, the first image could be 256x64, then
the sequence would be 128x32, 64x16, 32x8, 16x4, 8x2, 4x1, 2x1,
1x1.  You can stop producing images at any point in this
sequence, and the system will interpolate the last one for 
smaller renderings.  This system is used in the 
`r linkfn("legend3d", pkg = "rgl")` function to make text more legible in
smaller renderings.

If `mipmap = TRUE` is used without specifying a sequence of
images, `rgl` will compute the sequence itself:  this slows
loading a little, but speeds later rendering.  See 
`r linkfn("material3d", pkg = "rgl")`  for a discussion of the `texminfilter`
and `texmaxfilter` parameters that control the interpolation.
...
The in-development [`rgl2gltf`](https://dmurdoch.github.io/rgl2gltf/dev)
package can read or write GLTF and GLB files.
