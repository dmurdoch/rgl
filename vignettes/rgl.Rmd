...
If you call `r citefn("plot3d")` again, it will overwrite the current plot.
To open a new graphics window, use `r citefn("open3d")`.
...
`r indexfns(c("surface3d", "terrain3d"))`:    | a surface (as used in `r citefn("persp3d")`)
...
as necessary.  For example, the `r citefn("triangles3d")` function takes each
...
only work within R; in HTML output from `r citefn("writeWebGL")` only
