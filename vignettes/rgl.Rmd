...
```{r persp3d, rgl=TRUE, fig.height=3, fig.width=6}
library(MASS)
...
```{r rgl=TRUE, fig.width=3, fig.height=3}
...
`r indexfns("writeWebGL")` | Write HTML and Javascript to display a scene in a web browser.  (This vignette uses `writeWebGL`; see also [User Interaction in WebGL](WebGL.html).)
...
The `knitr` package has functions 
`r linkfn("hook_rgl", text="knitr::hook_rgl", pkg="knitr")` and 
`r linkfn("hook_webgl", text="knitr::hook_webgl", pkg="knitr")` 
...
These functions also write HTML and Javascript, for working with the 
