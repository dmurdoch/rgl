...
```{r "persp3d()", fig.height=3, fig.width=6, fig.keep="last", eval=requireNamespace("MASS",quietly=TRUE)}
...
`r indexfns("latex3d")`:      | render LaTeX fragments
...
```{r "triangles3d()", fig.width=3, fig.height=3}
...
defunct
functions 
`r deprecated(c("rgl.abclines", "rgl.bbox", "rgl.bg",
"rgl.clipplanes", "rgl.lines", 
"rgl.linestrips", "rgl.planes", "rgl.points",
"rgl.primitive", 
"rgl.quads", "rgl.setAxisCallback",
"rgl.spheres", "rgl.sprites", "rgl.surface",
"rgl.texts", 
"rgl.triangles"))`. 
These functions were used in earlier versions of
`rgl` but no longer work.  See the 
`r linkfn("r3d", text="?r3d", pkg="rgl")` help
topic for details.  

The function
`r indexfns(c("rgl.getAxisCallback"))`
provides low-level support for `r linkfn("setAxisCallbacks")`.
...
You can change the camera angle simply by dragging the picture with
the mouse.

To set the camera angle programmatically, use `r indexfns("view3d")`. 
This uses polar coordinates:

* `theta` sets the rotation around the vertical axis, in degrees. 
* `phi` sets the "vertical" rotation around the horizontal axis, 
  from -90 to 90 degrees.
  
The default angle is roughly `theta = 0, phi = -70`. Starting from
this position:

* As you increase `theta`, the graph will spin anticlockwise from your
  point of view.
* As you increase `phi` to 0, you start to look down at the scene
  from the top. If you increase `phi` above 0, you continue over and 
  start to see the graph from the "back" (and upside down).

You can also use `r linkfn("observer3d")` to change the camera 
...
The defunct `r deprecated("rgl.light")` function can
no longer be used; use `r linkfn("light3d")` instead.
...
`r linkfn("material3d", text="?material3d", pkg="rgl")` help page, and listed in the `r indexfns("rgl.material.names")`
variable.  All of them can be set except the ones in 
`r indexfns("rgl.material.readonly")`.
Here we give an overview.

Property  | Default | Meaning 
--------- | ------- | ------------------
color     | white   | vector of surface colors to apply to successive vertices for diffuse light
alpha     | 1       | transparency ranging from  0 (invisible) to 1 (opaque)
...
textures  | NULL   | a list of texture specifications; see below
front, back | fill  | should polygons be filled, or outlined? 
size      | 3       | size of points in pixels
lwd       | 1       | width of lines in pixels

The `textures` property is a possibly empty named
list containing texture specifications.  These
may include a filename or array and various
parameters controlling the texture display.
Other properties include 
"texmipmap", "texmagfilter", "texminfilter", "texenvmap", "fog", 
"point\_antialias", "line\_antialias", "depth\_mask", "depth\_test", "polygon_offset", "margin", "floating", "tag" and "blend"; 
see `r linkfn("material3d", "the help page", pkg = "rgl")` for details.

### Textures

As described in the previous section, one of the material
properties is `textures`, a list of texture specifications.
Each texture specification is a named list entry,
possibly containing `filename`, the name of a bitmap file 
containing an image to be displayed on the surface, and other parameters 
controlling the display.  The default shaders
in `rgl` will only make use of one texture, but
user shaders could use more.

This section gives more details about textures.

Internally, `rgl` can only read PNG files for textures.  JPEG
files may be used with help from the `jpeg` package, and raster objects or arrays may be specified.
...
Some other notes:
 ...
 - The color in `quads3d()` above was specified to be white.  By default,
the colors in the bitmap will modify the color of the
surface.  If `col` is black (a common default), you won't see anything,
so a warning may be issued.  
 - You usually don't want specular 
 reflections (which show up as glare).  Setting `specular`
 to black prevents those.
 - The material property `"texmode"` allows texture colors to
be used differently.  The default is `"modulate"`, where the texture
values combine multiplicatively with the underlying values.
 - Another aspect of how the bitmap is handled is controlled
 by the material
 property `"textype"`.  The default is `"rgb"`, which 
 takes the red-green-blue colors from the bitmap and
 uses them to modify the corresponding colors in the polygon.  
 - Other possibilities for `"textype"` and `"texmode"` are described
...
Alternatively, font family names matching fonts
found on your system may be used.  Currently 
`rgl` uses the `fontconfig` and `Pango` libraries
to choose fonts; this may change in future
versions.
...
parameters, listed in the `r indexfns("rgl.par3d.names")`
variable.  All of them can be set except the ones in 
`r indexfns("rgl.par3d.readonly")`.  Some parameters are completely read-only; others are
...
modelMatrix | read-only | The OpenGL ModelView matrix; partly set by `r linkfn("view3d")`
...
attributes such as color or transformations to be specified.
...
This package was formerly on CRAN, and is perhaps still available on Github if
it has not returned to CRAN.
...
`r linkfn("as.mesh3d")`:   | a generic function; see below
...
The `r indexfns("checkDeldir")` function checks that a compatible version
...
tags         | (optional) A vector added by some functions (e.g. `r linkfn("clipMesh3d")`) to relate output parts to input parts.
...
`r indexfns("getBoundary3d")`: | get the boundary of a mesh object
...
```
  ...
  `highlevel()` if it starts a new one.  For example, suppose you have hypothetical functions `foreignHigh` and `foreignLow`:
  
```{r eval = FALSE}
foreignHigh()   # Produces a high level plot, but doesn't return
                # an appropriate value
highlevel()     # mark the output as high level
foreignLow()    # Modifies the previous plot
lowlevel()      # mark the output as low level
```  
...
the defunct `r deprecated("writeWebGL")` function to write HTML code to insert in a document. 
...
`r indexfns("hover3d")`:      | displays "hover" info about points
...
`r indexfns("as.rglscene")`: | Generic function, no methods in `rgl`

The [`rgl2gltf`](https://dmurdoch.github.io/rgl2gltf/dev/)
package can read or write GLTF and GLB files.  It includes an
`as.rglscene` method to convert GLTF objects to `rgl` scenes.  The
code in `rgl`'s `r indexfns("Buffer")` R6 class is based on the GLTF
format.  It is used by `r linkfn("rglwidget")` to make output
...
[RStudio](https://posit.co/).  Besides 
...
The functions `r indexfns(c("lowlevel", "highlevel", "rglId", "rgl.incrementID"))` are also for internal use,
...
`r indexfns("getShaders")` allows you to see what
...
`r indexfns("close3d")`:              | close the current window
`r indexfns("cur3d")`:                | id of the active device
`r indexfns("set3d")`:                | set a particular device to be active
`r indexfns("pop3d")`:     | delete objects from the scene
`r indexfns(c("clear3d"))`:            | delete all objects of certain classes
`r indexfns("ids3d")`:              | ids, types and tags of current objects
`r indexfns("tagged3d")`:                         | find tags or objects with tags

Some of these functions have alternate names for
back compatibility:  
`r deprecated(c("rgl.cur", "rgl.ids", "rgl.pop"))`. 
...
should not be called: `r deprecated(c("rgl.init", "rgl.quit"))`.  
...
```{r fig.alt="Volcano in rgl", echo = 2:3}
close3d()
...
```{r eval=requireNamespace("orientlib", quietly = TRUE) && requireNamespace("lattice", quietly = TRUE), fig.alt=paste0("Volcano in ", c("lattice", "base"), "graphics.")}
...
utility functions `r indexfns(c("in_pkgdown", "in_pkgdown_example"))` can be used
...
A known issue with `testthat` is that it doesn't
install a package before running the code; it
simulates installation using the
<<<<<<< Updated upstream
`devtools::load_all()` function.  This doesn't 
install files that are stored in a package's 
`inst` directory.  `rgl` depends on those `inst`
files, so may not behave properly in `testthat` 
tests.  A workaround is in place to load the
default shaders, but other content will not be
=======
`pkgload::load_all()` function.  This doesn't 
install files that are stored in a package's 
`inst` directory.  `rgl` depends on those `inst`
files (for shaders, textures and Javascript), 
so may not behave properly in `testthat` 
tests.  A workaround is in place to load the
default shaders, but other content may not be
>>>>>>> Stashed changes
found.
...
This section is for miscellaneous functions and objects 
that don't fall in any of the other categories in this document.
...
The `r indexfns("gltfTypes")` vector contains
constants used in OpenGL and glTF.

The `r indexfns("safe.dev.off")` function closes a base
graphics display and sets the previous display as current.

<<<<<<< Updated upstream
The `r indexfns(c("glyphAtlas", "rgl.textureRaster"))` functions are used for 
debugging `rgl`'s text handling.
=======
The functions `r indexfns(c("glyphAtlas", "rgl.textureRaster", "renderFromAtlas"))` 
are used for debugging `rgl`'s text handling.
>>>>>>> Stashed changes

These functions are defunct and no longer used:
`r deprecated(c("rgl.clear", "rgl.close",
"rgl.material", "rgl.open", "rgl.select3d", 
"rgl.set", "rgl.viewpoint"))`.
