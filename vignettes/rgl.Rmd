...
quads3d(xyz, texture = filename, texcoords = xyz[,c(1, 3)]*2, specular = "black")
```

Some other notes:

 - By default, the colors in the bitmap will replace the 
colors in the quad.  Prior to `rgl` version 1.0.6,
the colors in the bitmap modified the color of the
quad.  If the quad color was black (a common default), you wouldn't see anything.  To return to the previous behaviour,
set material property `texmode = "modulate"`.
 ...
 uses them to modify the corresponding colors in the polygon.  Other possibilities for `"textype"` and `"texmode"` are described
...
```{r eval=requireNamespace("orientlib", quietly = TRUE) && requireNamespace("lattice", quietly = TRUE)}
