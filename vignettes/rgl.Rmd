...
shade3d(tetrahedron3d(col="red")); next3d()
text3d(0,0,0,"cube3d"); next3d()
shade3d(cube3d(col="green")); next3d()
text3d(0,0,0,"octahedron3d"); next3d()
shade3d(octahedron3d(col="blue")); next3d()
text3d(0,0,0,"dodecahedron3d"); next3d()
shade3d(dodecahedron3d(col="yellow")); next3d()
text3d(0,0,0,"icosahedron3d"); next3d()
shade3d(icosahedron3d(col="orange")); next3d()
text3d(0,0,0,"cuboctahedron3d"); next3d()
shade3d(cuboctahedron3d(col="purple"))
```
...
rows <- len %/% cols
extra <- len %% cols
ofs <- c(0, rows + (extra > 0), rows + (extra > 1), rows + (extra > 2), rows)
ofs <- cumsum(ofs)
cat("<table><tbody>\n")
columns <- list()
for (i in seq_len(cols)) 
  columns[[i]] <- paste0('<td align="left" style="width:150px;">',
                         entries[(ofs[i]+1):(ofs[i]+rows)], '</td>')
cat(paste('<tr>', columns[[1]], columns[[2]], columns[[3]], columns[[4]], "</tr>"), sep="\n")  
if (extra) {
  cat("<tr>")
  for (i in seq_len(extra))
    cat('<td align="left">', entries[ofs[i+1]], '</td>')
  cat("</tr>\n")
}
cat("</tbody></table>\n")
```
