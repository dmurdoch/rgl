...
For example, the `r linkfn("persp3d")` example would be 
more informative if the range of log-likelihoods was
limited to values near the maximum (allowing visual
estimation of likelihood-ratio confidence regions). 
Currently `r linkfn("persp3d")` can do this by setting
out of range values to `NA`, but this yields a rough edge
in the plot.  A better result is available by adding a 
clipping plane in a `r linkfn("newSubscene3d", "subscene")`:
```{r rgl=TRUE}
zlim <- fit$loglik + c(-qchisq(0.95, 2)/2, 0)
plot3d(xlim, ylim, zlim, type = "n", 
       xlab = "shape", ylab = "rate", zlab = "log-likelihood")
newSubscene3d("inherit", "inherit", "inherit")
clipplanes3d(0, 0, 1, -zlim[1])
persp3d(loglik, xlim = xlim, ylim = ylim, n = 30, add = TRUE)
```
