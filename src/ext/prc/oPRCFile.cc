                               ...
                               uint32_t nC, const RGBAColour C[], const uint32_t CI[][2], const uint32_t SCI[], bool cu)
{
  ...
  if(vertex_color)
  {
    ...
    {
      if (segment_color) {
        tess->rgba_vertices.push_back(byte(C[SCI[i]].R));
        tess->rgba_vertices.push_back(byte(C[SCI[i]].G));
        tess->rgba_vertices.push_back(byte(C[SCI[i]].B));
        if(tess->is_rgba)
          tess->rgba_vertices.push_back(byte(0.5*C[SCI[i]].A));
      } else {
                               ...
                               bool segment_color, uint32_t nC, const RGBAColour C[], const uint32_t CI[][3], bool cu)
{
  ...
  if(vertex_color)
  {
    ...
    {
      for(uint32_t j=0; j<nV; j++) {
          tess->rgba_vertices.push_back(byte(C[CI[i][j]].R));
          tess->rgba_vertices.push_back(byte(C[CI[i][j]].G));
          tess->rgba_vertices.push_back(byte(C[CI[i][j]].B));
          if(tess->is_rgba)
            tess->rgba_vertices.push_back(byte(C[CI[i][j]].A));
      }
    }
  }
  const uint32_t tess_index = cu ? add3DWireTessUnique(tess) : add3DWireTess(tess);
