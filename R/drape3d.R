    ...
    plot = TRUE, P = cbind(diag(2), 0), ...) 
{
    ...
    ztri <- function(i){
      p <- psegs[,i]
        ## Finds triangle with point p inside of it and returns unprojected vector at this point
    ...
    z1 <- ztri(1)
    if (!is.null(z1)) result <- rbind(result, z1)
    ...
    for (i in seq_len(ncol(segs))[-1]){
      ...
      if (any(is.na(p1))) {
        ...
        result <- rbind(result, c(NA, NA, NA))
        zi <- ztri(i)
        if (!is.null(zi)) result <- rbind(result, zi)
        next
      }
      ...
      for(j in seq_len(nrow(tri))[!sx & !sy]){     ## possible intersections
        ...
        zs <- rbind(zs,c(t1,(v3+t2*v43)))## t1 along line seg & point value
      }
      if (nrow(zs)>0) {
        ## add points in order of increasing t on segment
        o <- order(zs[,1])
        result <- rbind(result, zs[o,-1])
      }
      
        ## add last segment point assuming not on triangle edge
        zn <- ztri(i)
        if (!is.null(zn)) result <- rbind(result, zn)

        if (plot) 
            ids <- c(ids, lines3d(result, ...))
    }
    if (plot)
      lowlevel(ids)
    else
