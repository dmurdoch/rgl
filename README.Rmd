...
This is the "shader" branch of the development version
of rgl.  The goal of this branch is to support OpenGL
3.3.  Currently development is taking place within
XQuartz on macOS, so functions are limited to OpenGL
2.1; however, I am avoiding fixed pipeline calls.

Still to do:

  - bug checking
  - documentation updates
  - adding a native macOS backend instead of using
  XQuartz, so 3.3 is available.
...
PLATFORMS
---------
macOS (with only partial support on Tahoe, as OpenGL support is currently deficient on that platform)  
Windows 7+  
Unix-derivatives  
...
Currently Tahoe, the most recent version of macOS, does not
support OpenGL in XQuartz, and it appears unlikely such 
