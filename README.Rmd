...
`rgl` can make use of development versions of some packages, currently just
`webshot2`. Though it isn't required, it provides
some nice features, supporting good quality PNG snapshots of `rgl`
scenes, even on servers that don’t have a graphics display.

Unfortunately, development versions of packages sometimes
that break `rgl` usage. Currently (May 18, 2022) the release candidate branch is
...
If you have problems with that version, the current
